...
import { favoritesStore, type FavoriteItem } from "./favorites";

function authed(request: Request) {
  return /mcc_session=/.test(request.headers.get("cookie") ?? "");
}

export const Route = createFileRoute("/api/favorites/toggle")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          const has = favoritesStore.has(body.movie_slug);
          if (has) {
            favoritesStore.delete(body.movie_slug);
            return Response.json({ ok: true, favorited: false });
          }
          favoritesStore.set(body.movie_slug, {
